...
class PlusbogBookScraper(BookScraper):
    """ Book scraper for Plusbog.dk store that uses POST request to external API. """
...
class SaxoBookScraper(BookScraper):
    """ Custom book scraper for Saxo.com store. """
    ...
    def find_book(self, isbn: str) -> SearchResult:
        self._rate_limiter.wait_if_needed()
        with HttpClient(headers=self._headers_for_search) as http_client:
            response = http_client.get(self._search_url.format(isbn))

        result = SearchResult(bookstore_id=self._bookstore_id, url=None, success=False)
        if response.redirected and response.url.endswith(isbn):
            result = replace(result, url=response.url, success=True)
