...
def test_deletes_multiple_images(config, images, images_deleted, book_image_file_service) -> None:
    ...
    job = DeleteUnusedImagesJob(config, mock_db, book_image_file_service)
    job.start()
...
def test_excludes_default_image_from_deletion(config, book_image_file_service) -> None:
    ...
    with open(os.path.join(config.imgdir, DeleteUnusedImagesJob.default_image_name), "wb") as default_image_file:
