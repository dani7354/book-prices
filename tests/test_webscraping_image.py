...
@pytest.mark.parametrize("css_selector,expected_url_found",
                         [("#img-folder-only", "http://example.com/static/images/books/1.jpg"),
                          ("#img-full-url", "http://example.com/static/images/books/1.jpg")])
def test_image_downloader_finds_url_from_img_element(monkeypatch, css_selector, expected_url_found):
    book_id = 1
    monkeypatch.setattr(requests, "get", shared.mock_get_image)
    image_downloader = ImageDownloader(".")
    image_downloader._get_image = MagicMock(return_value=f"{book_id}.jpg")
    image_source = ImageSource(book_id, "http://example.com/book1", css_selector, str(book_id))
